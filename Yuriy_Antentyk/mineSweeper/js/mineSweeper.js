...
const field__cell_modifiers = ["field__cell_opened", "field__cell_closed", "field__cell_flagged", "field__cell_bombed"];
for(let i = 1; i <= 8; ++i)
...
class Controller{
    static synchronizeCell (view, model){
        ...
        newModifiers.push(model.isDiscovered? "field__cell_opened": "field__cell_closed");
        if(model.isFlagged)
            newModifiers.push("field__cell_flagged");
        else if(model.isDiscovered){
            if(model.isMine)
                newModifiers.push("field__cell_bombed");
            else if(model.surroundingMines > 0){
                newModifiers.push("field__cell_number_" + String(model.surroundingMines));
                view.innerText = String(model.surroundingMines);
            }
        }

        for(let entry of field__cell_modifiers)
            view.classList.remove(entry);
        for(let entry of newModifiers)
            view.classList.add(entry);
    }

    static getFieldView() { return document.getElementById("field"); }
    static getCellView(i, j, fieldSize){ return this.getFieldView().getElementsByClassName("field__cell")[i * fieldSize + j]; }
    ...
    static synchronizeField(field){
        ...
        for(let i = 0; i < field.size(); ++i)
            for(let j = 0; j < field.size(); ++j)
                this.synchronizeCell(this.getCellView(i, j, field.size()), field.getCell(i, j));
    }
...
class Field{
    constructor(size){
        ...
        this.getCell = (i, j) => {
            if(!this.valid(i, j))
                throw RangeError("Invalid property range");
            return _field[i][j];
        };

        let _initMines = () => {
            let minesMap = (new Array(_size)).fill(0).map(x => (new Array(_size).fill(false)));
...
class Game{
    ...
    _discover(i, j){
        if(!this.field.valid(i, j) || this.field.getCell(i, j).isDiscovered || this.field.getCell(i, j).isFlagged)
            return;
        this.field.getCell(i, j).isDiscovered = true;
        if(this.field.surroundingMines(i, j) == 0 && !this.field.getCell(i, j).isMine)
            for(let d = 0; d < di.length; ++d)
                this._discover(i + di[d], j + dj[d]);
                    
        Controller.synchronizeCell(Controller.getCellView(i, j, this.field.size()), this.field.getCell(i, j));
    }
    ...
    flag(i, j){
        if(!this.field.valid(i, j) || this.field.getCell(i, j).isDiscovered)
            return;
    ...
    checkLose(){
        for(let i = 0; i < this.field.size(); ++i)
            for(let j = 0; j < this.field.size(); ++j)
                if(this.field.getCell(i, j).isMine && this.field.getCell(i, j).isDiscovered)
                    return true;
        return false;
    }
    ...
    handleEndOfTheGame(){
        if(this.checkWin() || this.checkLose()){
            ...
            if(this.checkLose()){
                for(let i = 0; i < this.field.size(); ++i)
                    for(let j = 0; j < this.field.size(); ++j)
                        if(this.field.getCell(i, j).isMine)
                            this._discover(i, j);
                Controller.synchronizeField(this.field);
            }
            setTimeout(() => {
