...
class Projects extends Component {
  ...
  constructor(props) {
    ...
    this.setState({
      visible: urlSplit.length === 1 ? 'projects' : urlSplit[1],
    });
      ...
      <React.Fragment>
        <section className={b() + ' ' + additionalClassName}>
          <nav className={b('project-type-switchers')}>
            ...
            <a
              href="#internships"
              onClick={() => this.onInternshipsClick()}
              className={
                visible === 'internships'
                  ? b('switch', ['active'])
                  : b('switch')
            ...
            <a
              href="#volunteering"
              onClick={() => this.onVolunteeringClick()}
              className={
                visible === 'volunteering'
                  ? b('switch', ['active'])
                  : b('switch')
          ...
          <section className={b('wrapper')}>
            {(() => {
              if (visible === 'projects') {
                return workProjects.map(function(project) {
                  return (
                    <React.Fragment>
                      <Project
                ...
                return internships.map(function(project) {
                  return (
                    <React.Fragment>
                      <Project
                        key={project.projectId}
                ...
                return volunteering.map(function(project) {
                  return (
                    <React.Fragment>
                      <Project
                        key={project.projectId}
