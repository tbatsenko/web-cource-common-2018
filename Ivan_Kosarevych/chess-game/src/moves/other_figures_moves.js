...
function twoZerosMove(player) {
  function kingMove(player) {
    ...
    return [
      [boardRowNames[row], boardColumnNames.E],
      [boardRowNames[row], boardColumnNames.G]
    ];
  }
  ...
  function rockMove(player) {
    let row = player === "w" ? 1 : 8;
    return [
      [boardRowNames[row], boardColumnNames.H],
      [boardRowNames[row], boardColumnNames.F]
    ];
  }
...
function threeZerosMove(player) {
  function kingMove(player) {
    ...
    return [
      [boardRowNames[row], boardColumnNames.E],
      [boardRowNames[row], boardColumnNames.C]
    ];
  }
  ...
  function rockMove(player) {
    let row = player === "w" ? 1 : 8;
    return [
      [boardRowNames[row], boardColumnNames.A],
      [boardRowNames[row], boardColumnNames.D]
    ];
  }
